/**
 * AI Service for Lead Enrichment using OpenAI
 * Converted to TypeScript with proper type definitions
 */

import OpenAI from 'openai';
import { ScrapedData, AIEnrichmentResult, Lead } from '../types';
 ...
 */
const AI_PROMPTS = {
    leadEnrichment: (data: ScrapedData): string => `
...
Return ONLY valid JSON with this exact structure:
{
    ...
    websiteMetadata: (url: string, data: ScrapedData): string => `
...
Return JSON with:
{
    ...
    textParsing: (text: string): string => `
 ...
 */
export function validateAndCleanFields<T extends CleanedFields>(data: T): T {
    const cleaned = { ...data };
...
interface EnrichOptions {
    type?: 'leadEnrichment' | 'website' | 'text';
}

interface RawDataWithUrl extends ScrapedData {
    url?: string;
    text?: string;
}
...
export async function enrichLeadWithAI(
    ...
    try {
        ...
        const completion = await client.chat.completions.create({
            ...
            model: "gpt-3.5-turbo", // Using gpt-3.5-turbo for wider access compatibility
        });
        ...
        const cleanedResult = validateAndCleanFields(result as CleanedFields);
...
export async function extractWebsiteMetadata(
    ...
    try {
        ...
        return metadata as unknown as WebsiteMetadata;
    } catch (error) {
 ...
 */
export async function generateAutoTags(lead: Lead): Promise<string[]> {
