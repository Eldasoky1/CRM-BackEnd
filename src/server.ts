...
// Import services
import { scrapeProfile } from './services/scraper';
import { calculateLeadScore, findDuplicates, leadsToCSV } from './utils/week2Utils';
import { enrichLeadWithAI, generateAutoTags } from './services/ai';
...
import { startScheduler, getQueueStatus, addToQueue } from './services/jobs/enrichmentWorker';
import { enrichWithWaterfall } from './services/enrichment/waterfall';
import { batchEnrichByUser, batchEnrichByIds } from './services/enrichment/batchEnrichment';
...
// Load OpenAPI spec
let swaggerDocument: object | undefined;
try {
...
app.post('/api/scrape', async (req: Request, res: Response) => {
    const { userId, targetUrl, platform } = req.body as ScrapeRequest;
    ...
    try {
        ...
        try {
            ...
            await supabase.from('scrape_jobs').update({
                status: 'failed',
                error_log: (scrapeErr as Error).message
            }).eq('id', job.id);
            throw scrapeErr;
        }
...
/**
 * SEARCH LEADS (BUS-96)
 * IMPORTANT: This route MUST come before /api/leads/:userId to avoid matching conflicts
 */
...
app.get('/api/leads/:userId', async (req: Request, res: Response) => {
    ...
    if (minScore) query = query.gte('lead_score', parseInt(minScore as string));
 ...
 * MOVED HERE: Must be before :userId/:leadId to prevent 'export' matching as leadId
 */
app.get('/api/leads/:userId/export', async (req: Request, res: Response) => {
    const { userId } = req.params;
 ...
 */
app.get('/api/leads/:userId/:leadId', async (req: Request, res: Response) => {
 ...
 */
app.patch('/api/leads/:leadId', async (req: Request, res: Response) => {
 ...
 */
app.post('/api/leads/parse', async (req: Request, res: Response) => {
...
app.post('/api/leads/import', async (req: Request, res: Response) => {
    ...
    try {
        const leadsToInsert = leads.map((lead: Record<string, string>) => ({
            user_id: userId,
 ...
 */
app.post('/api/leads/:leadId/enrich', async (req: Request, res: Response) => {
    const { leadId } = req.params;
    ...
    try {
        ...
        const newScore = calculateLeadScore({ ...lead, ...enrichedData } as Lead);
 ...
 */
app.post('/api/leads/:leadId/score', async (req: Request, res: Response) => {
    const { leadId } = req.params;
    ...
    try {
        ...
        const score = calculateLeadScore(lead as Lead);
 ...
 */
app.post('/api/leads/dedupe', async (req: Request, res: Response) => {
 ...
 */
app.post('/api/leads/bulk', async (req: Request, res: Response) => {
...
app.get('/api/tags/:userId', async (req: Request, res: Response) => {
    const { userId } = req.params;
...
app.post('/api/leads/:leadId/tags', async (req: Request, res: Response) => {
    const { leadId } = req.params;
    const { tagIds } = req.body;

    try {
        const inserts = tagIds.map((tagId: string) => ({ lead_id: leadId, tag_id: tagId }));
 ...
 */
app.post('/api/leads/:leadId/auto-tag', strictLimiter, asyncHandler(async (req: Request, res: Response) => {
    const { leadId } = req.params;

    const { data: lead } = await supabase.from('leads').select('*').eq('id', leadId).single();
    if (!lead) return res.status(404).json({ error: 'Lead not found' });

    const suggestedTags = await generateAutoTags(lead as Lead);
 ...
 */
app.post('/api/webhooks', asyncHandler(async (req: Request, res: Response) => {
    const { userId, url, events, secret } = req.body as WebhookRequest & { userId: string };
    const webhook = webhookManager.register(userId, { url, events, secret });
    res.status(201).json({ webhook });
}));

app.get('/api/webhooks/:userId', (req: Request, res: Response) => {
    const webhooks = webhookManager.getByUser(req.params.userId);
    res.json(webhooks);
});

app.delete('/api/webhooks/:webhookId', (req: Request, res: Response) => {
...
app.post('/api/queue/enrich', asyncHandler(async (req: Request, res: Response) => {
 ...
 */
app.post('/api/leads/batch-enrich', strictLimiter, asyncHandler(async (req: Request, res: Response) => {
    const { userId, leadIds, limit } = req.body as BatchEnrichRequest & { limit?: number };
...
// Wrap enrichWithWaterfall to match expected signature
const wrappedEnrich = async (lead: {
    email?: string | null;
...
startScheduler(supabase, wrappedEnrich, (lead: Lead) => {
