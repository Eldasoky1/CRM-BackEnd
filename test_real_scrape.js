...
async function main() {
    ...
    // 2. Test with a real, publicly accessible profile
    console.log("\n🚀 Sending POST request to http://localhost:5000/api/scrape ...");
    console.log("📌 Testing with Wikipedia page (to avoid LinkedIn auth wall)...\n");

    const payload = {
        userId: userId,
        targetUrl: 'https://en.wikipedia.org/wiki/Elon_Musk',
        platform: 'web'
    };
    ...
    try {
        ...
        if (response.ok) {
            console.log("\n✅ SUCCESS! Real scraping + AI enrichment is working!");
        }
